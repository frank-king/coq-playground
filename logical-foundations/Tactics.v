...
(** **** Exercise: 3 stars, standard (apply_exercise1)
  ...
  Lemma rev_injective_inv : forall (l1 l2 : list nat),
    l1 = l2 -> rev l1 = rev l2.
  Proof.
    intros l1 l2 H.
...
(** 
   - [apply]: consumes goal [Q] using a proved theorm  [P -> Q].
   - [rewrite]: transforms a term [x] to [y] using a proved theorm [x = y].
    [] *)
...
Proof.
  intros n. induction n as [| n' IHn'].
  - intros m H. symmetry. apply eqb_0_l. apply H.
  - intros [| m'] H.
    + discriminate.
    + simpl in H.
...
(** Proof: By induction on [n].
   ...
   - When [n = 0]. By a straight-forward induction on [n] we can obtain
  
   forall n, n =? 0 = true -> n = 0.

   By symmetry, we have n =? 0 = true -> 0 = n.

   - When [n = S n']. We must show that
  
   S n' =? m = true -> S n' = m.

   It is obviously that [m] can only be formed [m = S m']. That follows to

   S n' =? S m' = true -> S n' = S m'.

   By a simple injection, we have:

   n' =? m' = true -> n' = m'.

   Which is immediately obtained from the induciton hypothesis.  [] *)
...
Proof.
  ...
  - intros l1 l2 H. injection H as H1 H2. rewrite <- H1. reflexivity.
  - intros [| x' l1'] [| y' l2'] H.
    + simpl in H. destruct (split l'). injection H as H. discriminate H.
    + simpl in H. destruct (split l'). injection H as H. discriminate H.
    + simpl in H. destruct (split l'). injection H as _ _ H. discriminate H.
    + simpl in H. destruct (split l'). injection H as H1 H1' H2 H2'.
      rewrite H1.
      rewrite H2.
      simpl.
      apply f_equal.
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal)
   ...
   Proof: By an induciton on [n].

   - Suppose [n = 0]. In the case [m = 0], we get [(0 =? 0) = (0 =? 0)],
     which is immediate by the reflexivity. In the case [m = S m'], 
     we get [(0 =? S m') = (S m' =? 0)], and by the definition of [=?],
     we have [false = false], which is also immediate by the reflexivity.

   - Suppose [n = S n']. We must show that [(S n' =? m) = (m =? S n')].
     In the case [m = 0], by the definition of [=?], this follows to
     [false = false], which is immediate by the reflexivity. In the case
     [m = S m'], by the definition of [=?], this follows to
     [(n' =? m') = (m' =? n')], which is immediate by the induction hypothesis.
     *)
   (* [] *)
...
Proof.
  intros n. induction n as [| n' IHn'].
  - intros m p H H0. apply eqb_0_l in H. rewrite H in H0. apply H0.
  - intros [| m'] [| p'] H1 H2. 
    + simpl in H1. discriminate H1.
    + simpl in H1. discriminate H1.
    + simpl in H2. discriminate H2.
    + simpl. simpl in H1. simpl in H2. apply (IHn' m' p').
      * apply H1.
      * apply H2.
Qed.
...
Lemma length_cons : forall (X : Type) (l : list X) (x : X),
  length (x :: l) = S (length l).
Proof.
  intros X l x. induction l as [| h t IHl'].
  - reflexivity.
  - reflexivity.
Qed.
...
Proof.
  intros X Y l1. induction l1 as [| x l1' IHl1'].
  - intros [| y l2'] H.
    + reflexivity.
    + unfold length in H. discriminate H.
  - intros [| y l2'] H.
    + unfold length in H. discriminate H.
    + rewrite length_cons in H.
      ...
      injection H as H.
      simpl.
      replace (split (combine l1' l2')) with (l1', l2').
      { reflexivity. }
      simpl.
      symmetry.
      apply IHl1'.
...
Theorem filter_exercise : forall (X : Type) (test : X -> bool)
  ...
  test x = true.
Proof.
  intros X test x l lf. induction l as [| h l' IHl'].
  generalize dependent x.
  - discriminate.
  - intros H. unfold filter in H. destruct (test h) eqn:Heq.
    + injection H as H H0. rewrite <- H. apply Heq.
    + apply IHl'. apply H.
Qed.
(** [] *)
